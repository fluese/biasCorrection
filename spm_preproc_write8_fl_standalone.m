...
for n=1:N,
    ...
    if bf(n,2),
        chan(n).Nc      = nifti;
        %chan(n).Nc.dat  = file_array(fullfile(pth1,[nam1, '_fwhm-' num2str(biasfwhm) '_reg-' num2str(biasreg) '_samp-' num2str(samp) '.nii']),...            % fl
        chan(n).Nc.dat  = file_array(fullfile(pth1,[nam1 '_biasCorrected.nii']),...            % fl
                                     res.image(n).dim(1:3),...
                                     [spm_type('uint16') spm_platform('bigend')],...
                                     0,1,0);
    ...
    if bf(n,1),
        chan(n).Nf      = nifti;
        chan(n).Nf.dat  = file_array(fullfile(pth1,[nam1, '_Biasfield.nii']),...            % fl
                                     res.image(n).dim(1:3),...
