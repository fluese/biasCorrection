...
for iter=1:nit
	...
	res = spm_preproc8(obj);
        ...
        if iter==nit
            ...
            spm_preproc_write8_fl_standalone(res,tmp1,tmp2,tmp3,job.warp.mrf,job.warp.cleanup,job.warp.bb,job.warp.vox, job.channel.biasfwhm, job.channel.biasreg, job.warp.samp);  % fl
        else
            % Not the final iteration, so compute sufficient statistics for
            % re-estimating the template data.
            N    = numel(job.channel);
            K    = numel(job.tissue);
            [cls,M1] = spm_preproc_write8_fl_standalone(res,zeros(K,4),zeros(N,2),[0 0],job.warp.mrf,...                   % fl
                                          job.warp.cleanup,job.warp.bb,job.warp.vox, job.channel.biasfwhm, job.channel.biasreg, job.warp.samp);
